#!/usr/bin/env bash
# v28b: same as v28a (1.7B, LME, F3 off) but seed=2.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v28b_no_probe_seed2_1p7b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v28b_no_probe_seed2_1p7b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v28b_no_probe_seed2_1p7b_frozen_gh200.log" 2>&1
echo "v28b finished"
