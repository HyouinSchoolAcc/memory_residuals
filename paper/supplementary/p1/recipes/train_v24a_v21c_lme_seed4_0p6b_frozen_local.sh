#!/usr/bin/env bash
# v24a-seed4: identical to v24a but seed=4. Fourth seed for the headline CI.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v24a_v21c_lme_seed4_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 4 \
    --run_name chain_v24a_v21c_lme_seed4_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v24a_v21c_lme_seed4_0p6b_frozen_local.log" 2>&1 &
echo "v24a_seed4 launched pid=$!"
