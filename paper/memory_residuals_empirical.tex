...
\If{negative-chain auxiliary loss is enabled}
  \State sample a different chain's window; build shuffle $\Mc^{\text{sh}}$
  \State $L_{\text{sh}} \gets$ NLL of $C_k$ under $\Mc^{\text{sh}}$
  \State $L \gets L + \lambda \cdot \max(0, L_k - L_{\text{sh}} + m)$
\EndIf
...
from training by user mandate. We have no Phase-2 conversational SFT
result yet; the architecture's behaviour under instruction tuning is
unknown. The shuffle test as currently implemented compares against
``a different chain in the same corpus'' -- a stronger test would be
``a different chain in the same domain but with adversarially
