#!/usr/bin/env bash
# v30a: v28a recipe (1.7B, F3-off, frozen Qwen3-1.7B) trained on synthd5
# instead of LME. Tier-2 (4) at scale: same hypothesis as v29a but on
# the larger backbone, where we have more representational headroom for
# binding random alphanumeric IDs into M_c.
#
# Diff from v28a/seed1: --train_chains flips from LME to synthd5,
# --eval_chains stays on synthd5_val (in-domain eval), --steps stays at
# 1500 (synthd5_train has 5000 chains, more than enough).
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v30a_synthd5_seed2_1p7b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v30a_synthd5_seed2_1p7b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v30a_synthd5_seed2_1p7b_frozen_gh200.log" 2>&1
echo "v30a finished"
