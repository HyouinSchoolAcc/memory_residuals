...
OUT_DIR="output/chain_v29a_synthd5_seed3_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 3 \
    --run_name chain_v29a_synthd5_seed3_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v29a_synthd5_seed3_0p6b_frozen_local.log" 2>&1 &
