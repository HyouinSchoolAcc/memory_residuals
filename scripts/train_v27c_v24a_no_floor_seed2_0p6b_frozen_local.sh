...
OUT_DIR="output/chain_v27c_v24a_no_floor_seed2_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v27c_v24a_no_floor_seed2_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v27c_v24a_no_floor_seed2_0p6b_frozen_local.log" 2>&1
echo "v27c-seed2 finished rc=$?"
