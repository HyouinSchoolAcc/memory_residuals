#!/usr/bin/env bash
# v24a-seed2: identical to v24a (0.6B, LME, F3 depth 4) but seed=2.
# Multi-seed reproducibility for the project headline (pa_cb_dnm).
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v24a_v21c_lme_seed2_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v24a_v21c_lme_seed2_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v24a_v21c_lme_seed2_0p6b_frozen_local.log" 2>&1 &
echo "v24a_seed2 launched pid=$!"
