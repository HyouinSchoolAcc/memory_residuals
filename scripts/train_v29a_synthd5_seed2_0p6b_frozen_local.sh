#!/usr/bin/env bash
# v29a: train-on-synthd5 with the v27b (F3-off) recipe.
# Tier-2 (4) of the friend's plan: "the most informative single
# experiment you have not run." Random alphanumeric IDs are
# unlearnable from a chain-class prior, so any positive Δ_cb here is
# proof that the architecture can bind chain-specific facts when the
# data forces it.
#
# Diff from v27b/seed1: --train_chains flips from LME to synthd5,
# --steps bumped 1000 -> 1500 (synthd5 has 5000 chains vs LME's 450),
# --eval_chains stays on synthd5_val (already the v27b setup).
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v29a_synthd5_seed2_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v29a_synthd5_seed2_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v29a_synthd5_seed2_0p6b_frozen_local.log" 2>&1 &
echo "v29a synthd5 seed=1 launched pid=$!"
