#!/usr/bin/env bash
# v32d: 1.7B sparse-writer twin of v32a. Top-k=8 slot-axis softmax on
# the judge SlotAttentionWriter, otherwise verbatim v30a/seed1 (synthd5
# train, F3-off, frozen Qwen3-1.7B). The pair to v32a (0.6B) so we can
# scale the architectural fix up.
#
# Diff from v30a/seed1: --memres_judge_topk_slot_softmax 8 (only flag).
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v32d_sparse_synthd5_seed1_1p7b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v32d_sparse_synthd5_seed1_1p7b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v32d_sparse_synthd5_seed1_1p7b_frozen_gh200.log" 2>&1
echo "v32d finished"
