#!/usr/bin/env bash
# v32a: v29a recipe (synthd5, F3-off, frozen Qwen3-0.6B) + Tier-3 (6)
# sparse-writer: top-k=8 slot-axis softmax in the judge SlotAttentionWriter.
#
# Diff from v29a/seed1: --memres_judge_topk_slot_softmax 8 (only flag).
# Tests the friend's load-bearing architectural recommendation: "Top-k
# sparse slot-axis softmax: replace the dense slot-axis softmax in
# Eq. \\ref{eq:writer-a} with a top-k that forces each candidate to bind
# to <= k slots. This breaks the smooth-mixing path that the chain-class
# prior probably exploits."
#
# K=128, k=8 means each input column distributes its mass across exactly
# 8 (6.25 %) of the 128 slots. v23/v29a's collapse mode was the
# uniform-symmetric fixed point (every slot collapsed to the same dense
# average); a top-k mask is structurally incompatible with that fixed
# point because at most 8 slots can carry any input's signature.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v32a_sparse_synthd5_seed1_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v32a_sparse_synthd5_seed1_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v32a_sparse_synthd5_seed1_0p6b_frozen_local.log" 2>&1 &
echo "v32a sparse-writer synthd5 seed=1 launched pid=$!"
