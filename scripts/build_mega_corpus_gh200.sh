...
#
# Pipeline:
#   (1) paper_tools/build_synthetic_dialogue_chains.py downloads each
#       HF source and produces per-chain JSONL files in
#       memory_residuals_data/mega_stage/<source>/<source>_<id>.jsonl
#   (2) paper_tools/pretokenize_chains.py tokenises each source into a
#       per-source .pt (parallel over 16 workers for speed)
#   (3) paper_tools/merge_chain_corpora.py concatenates every per-source
...
for SRC in "${SOURCES[@]}"; do
    ...
    if ! python -u paper_tools/build_synthetic_dialogue_chains.py \
        --sources "$SRC" \
        --out_dir "$STAGE_DIR" \
        --turns_per_session 4 \
...
for SRC in "${SOURCES[@]}"; do
    ...
    if ! python -u paper_tools/pretokenize_chains.py \
        --in_dir "$src_dir" \
        --out_path "$src_pt" \
        --tokenizer "$TOKENIZER" \
...
python -u paper_tools/merge_chain_corpora.py \
    "${MERGE_ARGS[@]}" \
    --out "$OUT_PT"
