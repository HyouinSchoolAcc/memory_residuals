#!/usr/bin/env bash
# v18b_f2f3_codes_0p6b_frozen_local (LOCAL GPU 1)
# F2 + F3 + alpha-floor composed: full architectural recipe.
#
# UPDATED 2026-05-03 16:15 UTC-5 -- the original v18b (no alpha
# floor; results moved to *.NOFLOOR.log / *.NOFLOOR/) revealed the
# next bottleneck at step 200: both probe losses dropping from
# ~12 -> ~6 (writer + readout encode chain content), but
# evidence_lift = -0.003 with alpha_mem_mean = 0.0087.  The depth
# router is keeping the memory channel almost closed -- chain-
# specific content in m_t never reaches the LM head.  See v18a's
# rationale block; v18b now matches v18a's recipe except for the
# writer-side F2 probe.
#
# Tests whether the writer-side and read-side probe channels
# compose additively or interfere ON TOP of the alpha floor.
# Predictions:
#   * F3 + alpha alone (v18a) sufficient -> v18b matches v18a.
#   * F2 + F3 + alpha needed (writer probe load-bearing) -> v18b
#     stronger than v18a.
#   * F2 conflicts with F3 (gradient interference on M_c) -> v18b
#     slightly worse than v18a.
#
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v18b_f2f3_codes_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES=1 nohup python -u src/train_chain.py \
    ...
    --run_name chain_v18b_f2f3_codes_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v18b_f2f3_codes_0p6b_frozen_local.log" 2>&1 &
echo "v18b launched pid=$!"
