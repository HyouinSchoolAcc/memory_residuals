...
OUT_DIR="output/chain_v30a_synthd5_seed1_1p7b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v30a_synthd5_seed1_1p7b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v30a_synthd5_seed1_1p7b_frozen_gh200.log" 2>&1
echo "v30a finished"
