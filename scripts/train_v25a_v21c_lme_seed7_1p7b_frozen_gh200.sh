#!/usr/bin/env bash
# v25a-seed7: Same recipe as v25a (1.7B, LME, F3 readout depth 4, floor 0.5)
# but seed=7 on GH200. Provides a second-seed 1.7B reading so we can report
# mean ± std for the scaling claim instead of a single point.
#
# Run on GH200 inside ~/memory_residuals after `source ~/venv/bin/activate`.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v25a_v21c_lme_seed7_1p7b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" nohup python -u src/train_chain.py \
    ...
    --seed 7 \
    --run_name chain_v25a_v21c_lme_seed7_1p7b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v25a_v21c_lme_seed7_1p7b_frozen_gh200.log" 2>&1 &
echo "v25a_seed7 launched pid=$!"
