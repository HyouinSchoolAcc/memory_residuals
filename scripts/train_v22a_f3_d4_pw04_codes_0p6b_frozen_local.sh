#!/usr/bin/env bash
# v22a_f3_d4_pw04_codes_0p6b_frozen_local (LOCAL GPU 0)
# v21c recipe + probe weight 0.4 (was 0.5). Brackets the probe-weight
# optimum from below. See Scripts/train_v22c_*.sh for the upper-
# bracket pair.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v22a_f3_d4_pw04_codes_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES=0 nohup python -u src/train_chain.py \
    ...
    --run_name chain_v22a_f3_d4_pw04_codes_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v22a_f3_d4_pw04_codes_0p6b_frozen_local.log" 2>&1 &
echo "v22a launched pid=$!"
