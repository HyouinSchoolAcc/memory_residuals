#!/usr/bin/env bash
# v27b-seed2: same as v27b (F3 OFF) but seed=2. Multi-seed for the
# surprising "F3 hurts" finding from v27b-seed1 (+0.80 vs v24a +0.16).
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v27b_v24a_no_probe_seed2_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v27b_v24a_no_probe_seed2_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v27b_v24a_no_probe_seed2_0p6b_frozen_local.log" 2>&1 &
echo "v27b_seed2 launched pid=$!"
