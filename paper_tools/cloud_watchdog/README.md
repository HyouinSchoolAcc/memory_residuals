...
cd ~/memory_residuals
chmod +x paper_tools/cloud_watchdog/*.sh
sudo apt-get install -y jq tmux curl                                # if missing
mkdir -p paper_tools/cloud_watchdog/{queue,running,done,failed,logs}
...
echo "$TOPIC" > paper_tools/cloud_watchdog/.ntfy_topic

# Start the daemon (detached). Logs go to logs/watchdog.log.
nohup paper_tools/cloud_watchdog/watchdog.sh \
    > paper_tools/cloud_watchdog/logs/watchdog.log 2>&1 &
echo "watchdog PID=$!"

# Optional: push a status heartbeat every 30 min.
nohup paper_tools/cloud_watchdog/heartbeat.sh "$TOPIC" 1800 \
    > paper_tools/cloud_watchdog/logs/heartbeat.log 2>&1 &
```
...
```bash
ssh ubuntu@192.222.50.225 \
  '~/memory_residuals/paper_tools/cloud_watchdog/enqueue.sh \
     niah_v3_softparity \
     "python paper_tools/niah_eval.py \
       --model output/chain_v3_softparity_full/best \
       --depths 1,5,10,20,30 \
       --output paper_artifacts/eval/niah_v3_softparity.json" \
     0 \
     "$(cat ~/memory_residuals/paper_tools/cloud_watchdog/.ntfy_topic)"'
```

Inside the cloud GPU you can also do the equivalent shorter form:

```bash
TOPIC=$(cat paper_tools/cloud_watchdog/.ntfy_topic)
paper_tools/cloud_watchdog/enqueue.sh niah_v3_softparity \
...
for d in queue running done failed; do
  printf '%-8s %s\n' "$d" "$(ls -1 paper_tools/cloud_watchdog/$d 2>/dev/null | wc -l)"
done

# Live tail of a running job
tail -f paper_tools/cloud_watchdog/logs/<job_name>.log
