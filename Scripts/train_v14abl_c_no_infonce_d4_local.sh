#!/usr/bin/env bash
# v14abl_c_no_infonce_d4_local (LOCAL H100 GPU 0, second pair) --
# v14 full minus InfoNCE. Isolates: does the contrastive callback
# signal wake the writer (gate_mean stuck at 0 in v13)?  See
# v14abl_a header for locked-in core (frozen backbone + alpha_mem
# floor + slot_attention writer + AP + slot_pos + orth queries).
set -eu
cd "$(dirname "$0")/.."
mkdir -p output logs
...
exec python -u src/train_chain.py \
    ...
    --run_name chain_v14abl_c_no_infonce_d4_local \
    --out_dir output/chain_v14abl_c_no_infonce_d4_local \
    2>&1 | tee logs/chain_v14abl_c_no_infonce_d4_local.log
