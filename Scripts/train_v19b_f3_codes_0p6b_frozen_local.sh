#!/usr/bin/env bash
# v19b_f3_codes_0p6b_frozen_local (LOCAL GPU 1)
# F3 + multi-layer readout depth=4 + strong alpha-floor.
#
# Capacity-ceiling pair to v19a. Same recipe as v19a but
# --memres_readout_depth 4 (4 additional residual cross-attn
# refinement layers stacked on the base MemoryReadout, total of 5
# cross-attn layers). v19a vs v19b bounds the marginal value of
# additional readout depth at the v19 frozen-backbone scale.
#
# Predictions before launch (committed to runs.md):
#   * v19a >> v18a in §5 ttt_lift_vs_floor: depth=2 is the right
#     intervention; promote to default.
#   * v19a >= v19b: capacity ceiling is at depth=2 for 0.6B frozen
#     on synthd5_random_codes; depth=4 trains slower and may
#     overfit the per-position refinement.
#   * v19b > v19a: capacity ceiling is higher; v20 candidate is
#     depth=8 + larger backbone.
#   * Both flat: read-side architecture itself (cross-attn) is the
#     wrong primitive; v20 should swap to linear-attn / DeltaNet.
#
# See Scripts/train_v19a_*.sh for the full design rationale.
#
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v19b_f3_d4_codes_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES=1 nohup python -u src/train_chain.py \
    ...
    --run_name chain_v19b_f3_d4_codes_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v19b_f3_d4_codes_0p6b_frozen_local.log" 2>&1 &
echo "v19b launched pid=$!"
