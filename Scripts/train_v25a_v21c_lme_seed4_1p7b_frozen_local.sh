#!/usr/bin/env bash
# v25a-seed4: 1.7B v25a (full recipe) seed=4 on local. Adds 1.7B headline data.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v25a_v21c_lme_seed4_1p7b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 4 \
    --run_name chain_v25a_v21c_lme_seed4_1p7b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v25a_v21c_lme_seed4_1p7b_frozen_local.log" 2>&1 &
echo "v25a_seed4 launched pid=$!"
