#!/usr/bin/env bash
# v25b: Same as v25a but train on LME+MSC merge (v11_lme_msc_train_s512.pt).
# Targets better OOD dialogue / MSC statistics before LoCoMo eval_chain.
#
# Run alongside v25a on a second GPU, e.g. CUDA_VISIBLE_DEVICES=1 bash ...
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v25b_v21c_lmemsc_seed1_1p7b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v25b_v21c_lmemsc_seed1_1p7b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v25b_v21c_lmemsc_seed1_1p7b_frozen_local.log" 2>&1 &
echo "v25b launched pid=$!"
