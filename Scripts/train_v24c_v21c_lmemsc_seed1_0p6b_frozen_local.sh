#!/usr/bin/env bash
# v24c: v21c recipe trained on LongMemEval (real-content data) instead
# of synthd5_random_codes (templated synthetic). Single-variable A vs
# v23a (which was v21c on synthd5 with seed=1).
#
# Rationale: LME has 11.2 M tokens at 99.9% real-content density (vs
# synthd5's 25.6 M at 5% density = 1.3 M effective). 8.6× more F3-
# supervised real-content tokens per step. If real-content training
# fixes the variance noise floor that v22/v23 exposed at probe weight
# 0.5, this is the canonical training corpus going forward.
#
# Eval still on synthd5_val so the metric is directly comparable to
# v18-v23 cells.
#
# Single-variable change vs v23a:
#   --train_chains paper_artifacts/chains/v11_lme_msc_train_s512.pt
#   (was paper_artifacts/chains/synthd5_random_codes_train_s512.pt)
#
# Same seed (1) as v23a so the variance comparison is clean.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v24c_v21c_lmemsc_seed1_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES=1 nohup python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v24c_v21c_lmemsc_seed1_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v24c_v21c_lmemsc_seed1_0p6b_frozen_local.log" 2>&1 &
echo "v24c launched pid=$!"
