#!/usr/bin/env bash
# v25a: Same recipe as v24a (v21c + LME train, F3 readout depth 4, floor 0.5)
# on Qwen3-1.7B-large with frozen backbone.
#
# Goal: verify the LME-trained memory pathway transfers at larger scale and
# sets up LoCoMo / MSC eval_chain numbers comparable to v14–v16 baselines.
#
# vs v24a (0.6B): batch 2 × grad_accum 4 (=8) matches effective batch 4×2;
# longer schedule (1500 steps, warmup 300) for optimizer stability on 1.7B;
# phase_aligned_eval_n_chains 32 to reduce peak VRAM during in-train eval.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v25a_v21c_lme_seed1_1p7b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" nohup python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v25a_v21c_lme_seed1_1p7b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v25a_v21c_lme_seed1_1p7b_frozen_local.log" 2>&1 &
echo "v25a launched pid=$!"
