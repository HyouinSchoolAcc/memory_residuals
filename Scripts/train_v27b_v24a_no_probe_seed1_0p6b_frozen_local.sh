#!/usr/bin/env bash
# v27b: v24a recipe with --readout_probe_loss_weight 0.0 (F3 OFF).
# Tests whether the chain-specific readout probe gradient is load-bearing.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v27b_v24a_no_probe_seed1_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}" nohup python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v27b_v24a_no_probe_seed1_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v27b_v24a_no_probe_seed1_0p6b_frozen_local.log" 2>&1 &
echo "v27b launched pid=$!"
