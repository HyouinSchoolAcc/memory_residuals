#!/usr/bin/env bash
# v28a: v25a recipe (1.7B, LME, depth=4, floor=0.5) but F3 readout-probe OFF.
# Tests whether the v27b-seed1 +0.798 finding (F3 off > F3 on at 0.6B)
# scales to 1.7B. Same backbone as v25a (frozen), same training corpus,
# same step budget.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v28a_no_probe_seed1_1p7b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 1 \
    --run_name chain_v28a_no_probe_seed1_1p7b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v28a_no_probe_seed1_1p7b_frozen_gh200.log" 2>&1
echo "v28a finished"
