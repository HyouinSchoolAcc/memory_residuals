#!/usr/bin/env bash
# v15f_d4v2_1p7b_jointtrain_local (LOCAL H100) -- 1.7B + joint train.
#
# 1.7B-large + the v14k recipe + UNFROZEN backbone (lr_backbone=2e-5).
# This is the v15 headline: does the architecture deliver evidence
# lift at non-trivial scale under realistic joint training?
set -eu
cd "$(dirname "$0")/.."
mkdir -p output logs
exec python -u src/train_chain.py \
    ...
    --run_name chain_v15f_d4v2_1p7b_jointtrain_local \
    --out_dir output/chain_v15f_d4v2_1p7b_jointtrain_local \
    2>&1 | tee logs/chain_v15f_d4v2_1p7b_jointtrain_local.log
