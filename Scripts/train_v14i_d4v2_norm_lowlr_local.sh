#!/usr/bin/env bash
# v14i_d4v2_norm_lowlr_local (LOCAL H100 GPU 0) -- norm + 3x lower LR.
#
# What this cell tests
# --------------------
# Even after the input-norm fix on extract C, the writer parameters
# (extract.W_K/V, judge.W_Q, slot queries) might still be over-driven
# by lr=1e-4 once the warmup router_bias is annealed away.  This cell
# repeats v14g but at lr=3e-5 (still 6x larger than the v13 backbone
# lr_backbone=2e-5; we're frozen-backbone so this is the only LR).
#
# If v14g converges fine and v14i only converges slower, the headline
# fix is sufficient and we can pick lr=1e-4 for paper.  If v14g
# oscillates or re-collapses post-anneal but v14i stays stable, the
# memory parameter manifold is sharper than expected and lr scheduling
# matters.
set -eu
cd "$(dirname "$0")/.."
mkdir -p output logs
...
exec python -u src/train_chain.py \
    ...
    --run_name chain_v14i_d4v2_norm_lowlr_local \
    --out_dir output/chain_v14i_d4v2_norm_lowlr_local \
    2>&1 | tee logs/chain_v14i_d4v2_norm_lowlr_local.log
