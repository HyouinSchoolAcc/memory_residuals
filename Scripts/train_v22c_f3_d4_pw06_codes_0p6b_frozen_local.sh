#!/usr/bin/env bash
# v22c_f3_d4_pw06_codes_0p6b_frozen_local (LOCAL GPU 1)
# v21c recipe + probe weight 0.6 (was 0.5). Upper bracket of the
# probe-weight sweep. See Scripts/train_v22a_*.sh for the lower
# bracket pair.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v22c_f3_d4_pw06_codes_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES=1 nohup python -u src/train_chain.py \
    ...
    --run_name chain_v22c_f3_d4_pw06_codes_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v22c_f3_d4_pw06_codes_0p6b_frozen_local.log" 2>&1 &
echo "v22c launched pid=$!"
