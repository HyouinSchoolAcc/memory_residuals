#!/usr/bin/env bash
# v27b-seed3: F3 readout-probe OFF, 0.6B, LME, seed=3.
# Third verification seed for the +0.798 v27b-seed1 finding.
set -euo pipefail
cd "$(dirname "$0")/.."

OUT_DIR="output/chain_v27b_v24a_no_probe_seed3_0p6b_frozen_gh200"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 3 \
    --run_name chain_v27b_v24a_no_probe_seed3_0p6b_frozen_gh200 \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v27b_v24a_no_probe_seed3_0p6b_frozen_gh200.log" 2>&1
echo "v27b_seed3 finished"
