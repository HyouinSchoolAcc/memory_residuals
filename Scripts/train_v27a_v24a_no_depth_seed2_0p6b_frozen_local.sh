#!/usr/bin/env bash
# v27a-seed2: v24a recipe with --memres_readout_depth 0 (no F3 refine layers).
...
OUT_DIR="output/chain_v27a_v24a_no_depth_seed2_0p6b_frozen_local"
LOG_DIR="logs"
mkdir -p "$LOG_DIR"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u src/train_chain.py \
    ...
    --seed 2 \
    --run_name chain_v27a_v24a_no_depth_seed2_0p6b_frozen_local \
    --out_dir "$OUT_DIR" \
    > "$LOG_DIR/chain_v27a_v24a_no_depth_seed2_0p6b_frozen_local.log" 2>&1
echo "v27a-seed2 finished rc=$?"
