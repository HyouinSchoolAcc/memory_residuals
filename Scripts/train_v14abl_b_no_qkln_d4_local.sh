#!/usr/bin/env bash
# v14abl_b_no_qkln_d4_local (LOCAL H100 GPU 1) -- v14 full minus judge QK-LN.
# Isolates: does post-projection RMSNorm on judge Q/K matter for breaking
# the D2 row-entropy/log(2K) = 0.998 collapse? See v14abl_a header for
# locked-in core.
set -eu
cd "$(dirname "$0")/.."
mkdir -p output logs
...
exec python -u src/train_chain.py \
    ...
    --run_name chain_v14abl_b_no_qkln_d4_local \
    --out_dir output/chain_v14abl_b_no_qkln_d4_local \
    2>&1 | tee logs/chain_v14abl_b_no_qkln_d4_local.log
