...
NOTE (2026-04-30 repo cleanup): the v2/v3 run logs and checkpoints
that the default arg values below point to were pruned on
2026-04-30. The default arg values are preserved for reference;
re-running this script requires either restoring those runs or
passing explicit --softparity_log / --attentionbase_log /
--simplegate_log / --*_ckpt arguments. The already-built figure
PDFs live in results/exp1_pair_recipe/figures/.

Inputs:
  - logs/chain_v3_softparity_full.log           (pruned)
  - logs/chain_v3_attentionbase_full.log        (pruned)
  - logs/chain_v2_abl_residual_mode.log         (pruned)
  - results/eval/*_eval.json
  - results/eval/*_horizon.json
  - results/eval/init_parity_test.json
...
def main() -> None:
    ...
    p.add_argument("--softparity_horizon",
                   default="results/eval/chain_v3_softparity_full_eval_horizon.json")
    p.add_argument("--attentionbase_horizon",
                   default="results/eval/chain_v3_attentionbase_full_eval_horizon.json")
    p.add_argument("--simplegate_horizon",
                   default="results/eval/chain_v2_abl_residual_mode_step5200_eval_horizon.json")
